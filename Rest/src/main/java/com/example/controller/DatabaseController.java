...
import com.example.request.ChangeColumnRequest;
...
public class DatabaseController {
    ...
    @PostMapping("/changeColumnName")
    @Operation(summary = "Change column name", description = "Changes column name")
    @ApiResponse(responseCode = "200", description = "Changes column name",
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = Table.class)))
    public ResponseEntity<?> changeColumnName(@Valid @RequestBody ChangeColumnRequest changeColumnRequest){
        dbHandler.changeColumnName(changeColumnRequest.getTableIndex(),changeColumnRequest.getColumnIndex(), changeColumnRequest.getName());
        Table tableDetails = new Table(dbHandler.getTablesData().get(dbHandler.getTablesData().size()-1).name);
        int tableIndex = dbHandler.getTablesData().size()-1;
        for (int i = 0; i < dbHandler.getColumns(tableIndex).size(); i++) {
            tableDetails.addColumn(dbHandler.getColumns(tableIndex).get(i));
        }
        for (int i = 0; i < dbHandler.getRows(tableIndex).size(); i++) {
